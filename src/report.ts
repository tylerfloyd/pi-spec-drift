...
export function renderMarkdown(ev: Evaluation, meta: ReportMeta): string {
  ...
  if (meta.prTitle) {
    lines.push(`> PR: ${meta.prTitle}`);
    lines.push("");
  }

  if (ev.verdict === "NOT_EVALUATED") {
    lines.push(
      "Spec drift could not be evaluated. Jev was not reached, so **nothing is being treated as clean**.",
    );
    lines.push("");
    if (ev.reason) {
  ...
  if (ev.verdict === "DRIFT") {
    ...
    lines.push("");
    lines.push(
      "Suggest either updating the spec to match the new behavior, or revising the change. This check is advisory and is not blocking.",
    );
    ...
    lines.push(
      `**Uncertain:** Jev is not decisive on ${names || "the change"}. A quick human look is recommended before merging. This check is advisory and is not blocking.`,
    );
  } else {
    lines.push(
      "No drift signals: the change and the spec appear to be consistent with each other.",
    );
  }
...
export function renderJson(
  ...
  if (ev.model) doc.model = ev.model;
  if (ev.reason) doc.reason = ev.reason;
...
function footer(lines: string[], ev: Evaluation, meta: ReportMeta): string {
  ...
  if (ev.model) bits.push(`model \`${ev.model}\``);
  if (ev.usage) {
